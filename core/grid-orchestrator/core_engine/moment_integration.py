...
_canonical_moment_logger = importlib.import_module("core_engine.mostar_moments_log")
...
class MostarMomentLog:
    ...
    def __init__(self):
        ...
        try:
            ...
            print("[INFO] Connected to Neo4j Grid Memory (Aura).")
    ...
    def record(self, moment: MostarMoment):
        ...
        # --- Neo4j log ---
        if not self.driver:
            print("Skipping Neo4j persistence (no driver).")
            return
        ...
        try:
            ...
            print(f"Neo4j moment write failed: {e}")
            self._reconnect()
...
def log_mostar_moment(
    initiator, receiver, description, trigger_type="ritual", resonance_score=1.0
):
    """
    Records a Mostar Moment — used by MoScriptEngine and other layers.
    """
    try:
        result = _canonical_moment_logger.log_mostar_moment(
            initiator=initiator,
            receiver=receiver,
