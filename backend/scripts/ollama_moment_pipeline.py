...
class OllamaMomentCapture:
    ...
    def _generate_quantum_id(self, model: str, prompt: str, timestamp: datetime) -> str:
        """Generate unique quantum ID for moment"""
        data = f"{model}:{prompt[:100]}:{timestamp.isoformat()}"
        hash_val = hashlib.sha256(data.encode()).hexdigest()[:16]
        return f"OLLAMA-{model.upper()}-{hash_val}"
    
    def _calculate_resonance(self, duration: float, eval_count: int, model: str) -> float:
        """
    ...
    async def generate_and_capture(self, 
                                    model: str, 
                                    prompt: str,
    ...
    def _log_moment(self, moment: OllamaMoment) -> Dict:
        ...
        quantum_id = self._generate_quantum_id(moment.model, moment.prompt, moment.timestamp)
    ...
    def _write_to_neo4j(self, moment_data: Dict):
        ...
        WITH m
        MATCH (layer:GridLayer {name: $layer})
        MERGE (m)-[:RESONATES_IN]->(layer)
        
        WITH m
        MATCH (prev:MoStarMoment)
        WHERE prev.timestamp < m.timestamp
          AND prev.quantum_id <> m.quantum_id
        WITH m, prev
        ...
        try:
            with self.driver.session() as session:
                session.run(query, **moment_data)
                print(f"✅ Logged moment {moment_data['quantum_id'][:30]}... to Neo4j")
...
class OllamaRouter:
    ...
    async def route_query(self, query: str, layer_hint: Optional[str] = None) -> Dict:
        ...
        result = await self.capture.generate_and_capture(
            model=model,
            prompt=query
        )
    ...
    def _classify_query(self, query: str) -> str:
        ...
        soul_keywords = ["soul", "spirit", "meaning", "purpose", "divine", "ifa", "oracle", "belief"]
        if any(kw in query_lower for kw in soul_keywords):
            return "dcx1-soul"
        
        # Body layer - action, execution
        body_keywords = ["code", "execute", "run", "build", "create", "implement", "action"]
